...
while i<=length(varargin), 
  ...
  if ischar(varargin{i}), 
    switch varargin{i}, 
     ...
     case 'con_v1', do_con_v1 = true;
     case 'con_v2', do_con_v2 = true;
...
for t = 1:trainlen, 
  ...
  if do_con_v1
      ...
      for i = 1:munits
          if conRestVector(bmu,1)>0
              ...
              conRestVector(conRestVector(1:end~=bmu,1)>0,1)=conRestVector(conRestVector(1:end~=bmu,1)>0,1)-delta;
              if conThreshVector(bmu,1)>conThreshold
                  conRestVector(bmu,1)=conRest;
              end
              break;
          end
      end
  ...
  elseif do_con_v2
      ...
      p = p + B.*(y - p);
      bi = C.*(1/munits  - p);
      [qerr bmu] = min((Dx.^2)*mask(known) - bi); % minimum distance(^2) and the BMU
      % tracking
      if tracking>0,
          track_table(ind) = sqrt(qerr);
          if ind==update_step,
