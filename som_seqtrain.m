...
while i<=length(varargin), 
  ...
  if ischar(varargin{i}), 
    switch varargin{i}, 
     ...
     case 'con_thresh', i=i+1; conThreshold = varargin{i};
     case 'con_rest', i=i+1; conRest = varargin{i};
     case 'delta', i=i+1; delta = varargin{i};
     case 'con', do_con = true;
     case 'radius', 
      i=i+1; 
      l = length(varargin{i}); 
      if l==1, 
...
conThreshVector = zeros(munits,1);
conRestVector = zeros(munits,1);
...
for t = 1:trainlen, 
  ...
  dist2 = (Dx.^2)*mask(known);
  dist2_1 = sort(dist2);
  minInd = 1;
  [qerr, ~] = min(dist2_1);
  bmu = find(dist2(:,1)==dist2_1(minInd,1), 1 );
  if do_con
      for i = 1:munits
          if conRestVector(bmu,1)>0
              minInd = minInd+1;
              bmu = find(dist2(:,1)==dist2_1(minInd,1), 1 );
              conRestVector(conRestVector(:,1)>0,1)=conRestVector(conRestVector(:,1)>0,1)-delta;
              conRestVector(conRestVector(:,1)<0,1)=0;
              continue;
          else
              conThreshVector(bmu,1)=conThreshVector(bmu,1)+delta;
              if conThreshVector(bmu,1)>conThreshold
                  if frequencyVector(bmu,1)>actFreq
                      activityDeltas(bmu,1)=activityDeltas(bmu,1)+actDelta;
                      activityDeltas(1:bmu-1,1)=activityDeltas(1:bmu-1,1)-actDelta;
                      activityDeltas(bmu+1:end,1)=activityDeltas(bmu+1:end,1)-actDelta;
                      activityDeltas(activityDeltas(1:end,1)<0)=0;activityDeltas(activityDeltas(1:end,1)>1)=1;
                  else
                      activityDeltas(1:end,1)=activityDeltas(1:end,1)-actDelta;
                  end
                  conRestVector(bmu,1)=conRest+conRest*activityDeltas(bmu,1);
              end
              break;
          end
      end
  end
%   [qerr bmu] = min(dist2); % minimum distance(^2) and the BMU
  ...
  % update M
  not_update = find(conRestVector(:,1)>0);
  to_update = setdiff(1:munits,not_update);
  M(to_update,known) = M(to_update,known) - h(to_update,ones(sum(known),1)).*Dx(to_update,:);
