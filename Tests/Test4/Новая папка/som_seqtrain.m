...
for t = 1:trainlen, 
  ...
  if do_con
      ...
      p = p + B*(y - p);
      bi = C*(1/munits - p);
      [qerr, bmu] = min((Dx.^2 - bi)*mask(known));
  end
...
%   M(:,known) = M(:,known) + alpha*(x(mu_x_1,known)-M(:,known)).*z;
